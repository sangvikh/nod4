# The μ-Core Architecture Manifesto & Hardware Primer (v1.3.0)
...
μ-Core specifies self-contained functional modules connected by standardized buses:
- **Accumulator & ALU Block** (`A`, `B`, `FLAGS`)
- **Instruction Counter Block** (`PR:PC`)
- **Data RAM Addressing Block** (`C:D`)
- **Hardware Stack Controller** (`SP`, `Stack RAM`)
...
μ-Core prioritizes absolute determinism:
...
```text
     ...
     └────────────▲─────────────┴────────────▲─────────────┘
                  │                          │
         ┌────────┴─────────┐       ┌────────┴─────────┐
         │  Register C Page │       │ Register D / Operand│
         │     (74HC574)    │       │   MUX (74HC157)  │
         └──────────────────┘       └──────────────────┘
```

The output of Register `C` wires directly to address pins `A15..A8` of the Data RAM chip. Address pins `A7..A0` are fed by a 2-to-1 Multiplexer (`74HC157`) switching between the instruction operand byte (for direct access) and Register `D` (for indirect access via `LOAD MAX` or `STORE MAX`).

---

### Trick #2: The `MAX` Dual Escape Sentinel

In μ-Core v1.3.0, the sentinel value **`MAX` (`$FF` for `W=8`)** provides two fundamental escape capabilities with zero extra opcode bloat:
...
### Trick #3: 2-Bit Opcode Class Decoding (ISA v1.3.0)
...
; 0x04    0x3     0x10      STORE 0x10     ; RAM[C:0x10] <- 0 (Initialize sum in RAM)
; 0x06    0x2     0xF0      LOAD 0xF0      ; A <- 5 (Fetch pre-stored literal $05)
; 0x08    0x1     0x01      MOV B, A       ; B <- 5 (Set B as loop index counter)
;
; --- LOOP HEAD (Offset 0x0A) ---
; 0x0A    0x2     0x10      LOAD 0x10      ; Fetch running sum into Accumulator A
; 0x0C    0x4     0x00      ALU ADD        ; A <- A + B (Add current loop index)
; 0x0E    0x3     0x10      STORE 0x10     ; Write updated sum back to RAM[C:0x10]
...
; 0x16    0x6     0x1A      JZ 0x1A        ; If ZF == 1, jump to DONE (Offset 0x1A)
; 0x18    0x5     0x0A      JMP 0x0A       ; Otherwise, jump back to LOOP HEAD
;
; --- DONE (Offset 0x1A) ---
; 0x1A    0x2     0x10      LOAD 0x10      ; Load final sum (15 / $0F) into Accumulator A
...
Pursuant to **ABI v1.3.0**, Register `A` passes arguments, Register `B` holds secondary arguments, and Register `C` is strictly callee-saved. Pre-stored literal `$07` resides at `$F0`.

```assembly
; Offset  Opcode  Operand   Mnemonic       Bus Behavior & Hardware State
; -------------------------------------------------------------------
; --- MAIN ROUTINE ---
; 0x00    0x2     0xF0      LOAD 0xF0      ; A <- 7 (Fetch parameter from $F0)
; 0x02    0x8     0x08      CALL 0x08      ; Hardware Stack[SP] <- 0x04; PC <- 0x08
; 0x04    0x3     0x20      STORE 0x20     ; Store returned result (14 / $0E) to RAM
...
; Offset  Opcode  Operand   Mnemonic       Bus Behavior & Hardware State
; -------------------------------------------------------------------
; 0x00    0x2     0xF0      LOAD 0xF0      ; A <- 42 (Fetch parameter)
; 0x02    0x1     0x01      MOV B, A        ; Pass argument in Register B (B <- 42)
; 0x04    0x2     0xF1      LOAD 0xF1      ; Load Page 2 target ID (2)
; 0x06    0x1     0x08      MOV C, A        ; C <- 2 (Set target Page ID)
; 0x08    0x2     0xF2      LOAD 0xF2      ; Load Page 2 entry offset (0x00)
; 0x0A    0x1     0x0C      MOV D, A        ; D <- 0x00 (Set target offset)
; 0x0C    0x5     0xFF      JMP MAX         ; Far Jump! PR <- C (2), PC <- D (0x00)
;
; --- RESUME ENTRY POINT (Offset 0x0E) ---
; 0x0E    0x2     0x05      LOAD 0x05      ; Fetch result (43 / $2B) stored by Page 2
...
; 0x04    0x3     0x05      STORE 0x05     ; Store result in RAM[C:0x05]
; 0x06    0x2     0xF3      LOAD 0xF3      ; Load Page 1 return ID (1)
; 0x08    0x1     0x08      MOV C, A        ; C <- 1 (Set return Page ID)
; 0x0A    0x2     0xF4      LOAD 0xF4      ; Load Page 1 return offset (0x0E)
; 0x0C    0x1     0x0C      MOV D, A        ; D <- 0x0E (Set return offset)
; 0x0E    0x5     0xFF      JMP MAX         ; Far Return! PR <- C (1), PC <- D (0x0E)
```
...
Now that you have the conceptual mental model and explicit bus semantics for **ISA v1.3.0**, you are ready to dive into the exact engineering standards or build software toolchains:

1. **Read the Formal ISA Specification (v1.3.0):** Complete bit-level definitions of all 16 opcodes, 16 ALU operations, flag semantics, and stack underflow/overflow rules.
2. **Read the Application Binary Interface (v1.3.0):** Register preservation rules, caller/callee contracts, shared mailbox structures, and assembly idioms.
...
**Welcome to μ-Core v1.3.0! Have fun building software and wiring chips!**
