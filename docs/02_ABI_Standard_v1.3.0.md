# μ-Core Software & ABI Specification (v1.3.0)
...
This specification defines the standard **Application Binary Interface (ABI)** and software conventions for programs running on μ-Core processors conforming to **ISA Specification v1.3.0**.
...
│ • Constant Materialization (LDI / LOAD)                               │
...
### 2.3 Local Subroutine Calling Convention
...
; ===================================================================
    LDI 0x2A            ; A <- Parameter (42)
    CALL ADD_FIVE       ; Local call (Pushes return PC to Hardware Stack)
    STORE 0x20          ; Store result waiting in A
...
Under **ISA v1.3.0**, hardware `CALL` and `RET` operations are strictly page-local (preserving Page Register `$PR`). Cross-page domain transfers and far jumps are executed natively using the `MAX` escape sentinel (`$FF` on μ8):

$$
\text{JMP MAX} \implies PR \leftarrow C, \quad PC \leftarrow D
$$

### 3.1 Cross-Page Subroutine / Far Return Convention
...
1. **Setup Far Return Address:** Prior to executing a far jump, the calling domain writes its Return Page ID (`$PR_{caller}`) and Return Offset (`PC_{return}`) into the designated Mailbox Control Block.
2. **Execute Far Jump:** The caller sets `$C \leftarrow C_{target}$` and `$D \leftarrow D_{target}$, then executes `JMP MAX`.
3. **Execute Far Return:** Upon completion, the target domain reads the caller's return destination into `C` and `D`, then executes `JMP MAX` to resume caller execution.

### 3.2 Shared Mailbox Protocol & Layout

Data Page `$C = $00$` offsets `$00..$0F` are designated as the **Shared System Mailbox**:


Shared System Mailbox Layout


| Offset | Field                     | Purpose                          |
|--------|---------------------------|----------------------------------|
| $00    | Syscall / Command ID      | Command Identifier               |
| $01    | Return Page ID (PR)       | Far Return Destination Page      |
| $02    | Return Offset (PC)        | Far Return Destination Offset    |
| $03    | Status / Response Code    | Execution Result Status          |
| $04..$0F | Parameter Payload       | Multi-byte Parameters            |
| $10..$FF | Module Private RAM      | Workspace for Kernel / Page 0    |
...
### 4.1 Constant Materialization

- **`LDI`** is the canonical assembler mnemonic for immediate loading, and is an alias for `LOAD` with a non-`MAX` operand.
- **Literal Data Pool Invariant:** Literal values are allocated sequentially within offsets **`$F0..$FE`** of the module's Data Page.

### 4.2 Multi-Byte Arithmetic Mechanics ($W=8$)

Pursuant to **ISA v1.3.0 (Flag Preservation Law)**, non-ALU instructions (`LOAD`, `STORE`, `MOV`, `PUSH`, `POP`) leave `CF` and `ZF` strictly untouched. Multi-byte addition and subtraction propagate `CF` cleanly across memory operations without requiring temporary flag storage:
...
; ===================================================================
    ; 1. Low Byte Addition
    LOAD 0x00           ; Read Low Byte 1 (Flags UNCHANGED)
    MOV B, A            ; B <- Low Byte 1 (Flags UNCHANGED)
    LOAD 0x02           ; Read Low Byte 2 (Flags UNCHANGED)
    ALU ADD             ; A <- Low1 + Low2 (Updates ZF and CF)
    STORE 0x04          ; Store Low Result (CF PRESERVED)

    ; 2. High Byte Addition with Carry
    LOAD 0x01           ; Read High Byte 1 (CF PRESERVED)
    MOV B, A            ; B <- High Byte 1 (CF PRESERVED)
    LOAD 0x03           ; Read High Byte 2 (CF PRESERVED)
    ALU ADD             ; A <- High1 + High2 + CF
    STORE 0x05          ; Store High Result
```
